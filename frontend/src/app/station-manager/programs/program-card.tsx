...
// Day names and recurrence labels are now loaded from translations inside the component
...
export function ProgramCard({ program: p }: { program:ProgramRowClient; managerId:string }) {
  const t = useTranslations("stationManager.programs.card");
  ...
  const DAY_NAMES = [t("daySunday"),t("dayMonday"),t("dayTuesday"),t("dayWednesday"),t("dayThursday"),t("dayFriday"),t("daySaturday")];
  const REC_LABELS: Record<string,string> = {
    DAILY:t("recDaily"), WEEKLY:t("recWeekly"), SELECTED_DAYS:t("recSelectedDays"), ONE_TIME:t("recOneTime"),
  };

  function ok(m:string)  { setMsg({type:"ok",  text:m}); setBusy(false); }
  function err(m:string) { setMsg({type:"err", text:m}); setBusy(false); }
  ...
  async function handleUpdate(e:React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    await run(updateProgram, new FormData(e.currentTarget), t("programUpdated"), () => setPanel("none"));
  }
  async function handleToggle(e:React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    await run(toggleProgram, new FormData(e.currentTarget), p.isActive ? t("programDisabled") : t("programEnabled"));
  }
  async function handleDisable(e:React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    await run(disableProgram, new FormData(e.currentTarget), t("programPermanentlyDisabled"));
  }
  ...
  async function handleAddSlot(e:React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    await run(createScheduleSlot, new FormData(e.currentTarget), t("slotAdded"), () => setAddSlotRuleId(null));
  }
  async function handleEditSlot(e:React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    await run(updateScheduleSlot, new FormData(e.currentTarget), t("slotUpdated"), () => setEditSlotId(null));
  }
  async function handleDeleteSlot(slotId:string) {
    const fd = new FormData(); fd.set("slotId",slotId); fd.set("programId",p.id);
    await run(deleteScheduleSlot, fd, t("slotDeleted"));
  }
    ...
    <div className="bg-slate-900 border border-slate-700/40 rounded-2xl overflow-hidden">
      ...
      <div className="px-5 py-4 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            <StatusBadge label={p.isActive ? t("statusActive") : t("statusInactive")} variant={p.isActive ? "success" : "neutral"} dot />
          </div>
        ...
        <div className="flex gap-2 flex-wrap flex-shrink-0">
          <button onClick={()=>{setPanel(panel==="edit"?"none":"edit");setMsg(null);}}
            ...
            {panel==="edit"?t("closePanel"):t("editBtn")}
          </button>
          <button onClick={()=>{setPanel(panel==="schedule"?"none":"schedule");setMsg(null);}}
            ...
            {panel==="schedule"?t("closePanel"):t("scheduleBtn")}
          </button>
          ...
          {p.isActive&&(
            <form onSubmit={handleDisable}>
              <input type="hidden" name="programId" value={p.id}/>
              <input type="hidden" name="stationId" value={p.stationId}/>
              <button type="submit" disabled={busy} className={btn("text-red-400 border-red-700/30 hover:border-red-500/50")}>{t("permanentDisable")}</button>
            </form>
          )}
        </div>
      </div>
      ...
      {panel==="edit"&&(
        <div className="border-t border-slate-700/50 bg-slate-800/40 px-5 py-5">
          <h3 className="text-sm font-semibold text-purple-300 mb-4">{t("editProgramData")}</h3>
          <form onSubmit={handleUpdate} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input type="hidden" name="programId" value={p.id}/>
            <input type="hidden" name="stationId" value={p.stationId}/>
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-slate-400 mb-1">{t("programTitleLabel")}</label>
            ...
            <div>
              ...
              <select name="isActive" defaultValue={p.isActive?"true":"false"} className={inp}>
                <option value="true">{t("statusActive")}</option><option value="false">{t("statusInactive")}</option>
              </select>
            </div>
      ...
      {panel==="schedule"&&(
        <div className="border-t border-slate-700/50 bg-slate-800/40 px-5 py-5 space-y-5">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-teal-300">{t("programSchedule")}</h3>
            <p className="text-[10px] text-slate-600">{t("conflictCheckNote")}</p>
          </div>

          {p.scheduleRules.length===0&&<p className="text-xs text-slate-600">{t("noRulesYet")}</p>}
            ...
            <div key={rule.id} className="bg-slate-800 border border-slate-700/40 rounded-xl p-4 space-y-3">
              ...
              <div className="flex items-start justify-between flex-wrap gap-2">
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 mr-2">· {rule.timezone} · {t("earlyEntry", { minutes: rule.allowConnectMinutesBefore })}</span>
                  {!rule.isActive&&<span className="text-[10px] text-orange-400 mr-1">({t("statusInactiveF")})</span>}
                </div>
                <div className="flex gap-1.5 flex-wrap">
                  <button onClick={()=>setEditRuleId(editRuleId===rule.id?null:rule.id)}
                    ...
                    {editRuleId===rule.id?t("closePanel"):t("editRuleBtn")}
                  </button>
                  <button onClick={()=>setAddSlotRuleId(addSlotRuleId===rule.id?null:rule.id)}
                    ...
                    {addSlotRuleId===rule.id?t("closePanel"):t("addSlotBtn")}
                  </button>
                </div>
              </div>
              ...
              {editRuleId===rule.id&&(
                <form onSubmit={handleEditRule} className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-slate-700/30">
                  ...
                  <input type="hidden" name="programId" value={p.id}/>
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-1">{t("recurrenceTypeLabel")}</label>
                    <select name="recurrenceType" defaultValue={rule.recurrenceType} required className={inpSm}>
                      ...
                      <option value="SELECTED_DAYS">{t("recSelectedDays")}</option>
                      <option value="ONE_TIME">{t("recOneTime")}</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-1">{t("timezoneLabel")}</label>
                    <input name="timezone" type="text" defaultValue={rule.timezone} dir="ltr" className={inpSm}/>
                  </div>
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-1">{t("earlyEntryMinutes")}</label>
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-1">{t("statusLabel")}</label>
                    <select name="isActive" defaultValue={rule.isActive?"true":"false"} className={inpSm}>
                      <option value="true">{t("statusActiveF")}</option><option value="false">{t("statusInactiveF")}</option>
                    </select>
                  </div>
              ...
              {/* Existing slots */}
              {rule.slots.length===0&&<p className="text-[10px] text-slate-600">{t("noSlots")}</p>}
              {rule.slots.map(slot=>{
                const day  = slot.dayOfWeek!==null?DAY_NAMES[slot.dayOfWeek]:"";
                const date = slot.slotDate?new Date(slot.slotDate).toLocaleDateString(dateLocale):"";
                  ...
                  <div key={slot.id} className="border-t border-slate-700/20 pt-2 space-y-2">
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <span className="text-xs text-slate-300">
                        {day||date||t("recDaily")} · {slot.startTime}–{slot.endTime}
                        {!slot.isActive&&<span className="mr-1 text-slate-600">({t("statusInactive")})</span>}
                      </span>
                      <div className="flex gap-1.5">
                        <button onClick={()=>setEditSlotId(isEditingSlot?null:slot.id)}
                          ...
                          {isEditingSlot?t("closePanel"):t("editSlotBtn")}
                        </button>
                      ...
                      <form onSubmit={handleEditSlot} className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 bg-slate-900/60 rounded-lg p-3">
                        ...
                        {(rule.recurrenceType==="WEEKLY"||rule.recurrenceType==="SELECTED_DAYS")&&(
                          <div className="col-span-2 sm:col-span-1">
                            <label className="block text-[10px] text-slate-500 mb-1">{t("dayLabel")}</label>
                        ...
                        {rule.recurrenceType==="ONE_TIME"&&(
                          <div className="col-span-2 sm:col-span-1">
                            <label className="block text-[10px] text-slate-500 mb-1">{t("dateLabel")}</label>
                            <input name="slotDate" type="date" defaultValue={slot.slotDate?new Date(slot.slotDate).toISOString().split("T")[0]:""} className={inpSm}/>
                          </div>
                        )}
                        <div>
                          <label className="block text-[10px] text-slate-500 mb-1">{t("fromLabel")}</label>
                          <input name="startTime" type="time" required defaultValue={slot.startTime} className={inpSm}/>
                        </div>
                        <div>
                          <label className="block text-[10px] text-slate-500 mb-1">{t("toLabel")}</label>
              ...
              {addSlotRuleId===rule.id&&(
                <form onSubmit={handleAddSlot} className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-slate-700/30">
                  ...
                  <div className="col-span-2 sm:col-span-4 flex justify-end gap-2">
                    ...
                    <button type="submit" disabled={busy} className="bg-teal-700 hover:bg-teal-600 disabled:opacity-50 text-white text-xs font-semibold rounded-lg px-4 py-1.5 transition-colors">{t("addSlot")}</button>
                  </div>
                </form>
              )}
            </div>
          ...
          {addingRule&&(
            <form onSubmit={handleAddRule} className="bg-slate-800 border border-slate-700/40 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <input type="hidden" name="programId" value={p.id}/>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">{t("recurrenceTypeLabel")}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">{t("timezoneLabel")}</label>
                <input name="timezone" type="text" defaultValue="Africa/Cairo" dir="ltr" className={inp}/>
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">{t("earlyEntryMinutes")}</label>
