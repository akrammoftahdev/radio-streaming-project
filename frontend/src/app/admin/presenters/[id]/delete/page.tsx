...
export default async function PresenterDeletePage({
  ...
  const depRows: DepRow[] = [
    // ── BLOCKING ─────────────────────────────────────────────────────────────
    {
      ...
      cleanupLabel:   t("disableAllPrograms"),
      cleanupConfirm: t("disableAllProgramsConfirm"),
      note: deps.activePrograms > 0
        ? t("activeProgramsBlocking", { count: deps.activePrograms })
        : deps.programs > 0
          ? t("inactiveProgramsInfo", { count: deps.programs })
          : undefined,
    },
    {
  ...
  const activeBlockers: string[] = [
    deps.activePrograms > 0 ? t("activeBlockerProgram", { count: deps.activePrograms }) : "",
    deps.recordings     > 0 ? t("activeBlockerRecording", { count: deps.recordings })   : "",
    deps.liveSessions   > 0 ? t("activeBlockerSession", { count: deps.liveSessions })    : "",
    deps.schedules      > 0 ? t("activeBlockerSchedule", { count: deps.schedules })      : "",
  ].filter(Boolean);

  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-6 md:p-10" dir={dir}>
      {/* ── Breadcrumb ── */}
      <div className="flex items-center gap-2 text-sm text-neutral-500 mb-8">
        <Link href="/admin" className="hover:text-neutral-300 transition-colors">{t("breadcrumbAdmin")}</Link>
        <span>/</span>
        <Link href="/admin/presenters" className="hover:text-neutral-300 transition-colors">{t("breadcrumbPresenters")}</Link>
        <span>/</span>
        <Link href={`/admin/presenters/${presenterId}/edit`} className="hover:text-neutral-300 transition-colors">
          {presenter.username}
        </Link>
        <span>/</span>
        <span className="text-red-400">{t("breadcrumbManageDelete")}</span>
      </div>
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-col items-end gap-1.5">
              <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${
                ...
                {presenter.isActive ? t("statusActive") : t("statusDisabled")}
              </span>
