...
export default async function EditProgramPage({
  ...
  const t = await getTranslations("admin.programs.edit");
  const tr = await getTranslations("admin.programs.recurrence");
  const locale = await getLocale();
  const dir = locale === "ar" ? "rtl" : "ltr";
  
  const DAY_NAMES = t.raw("days") as string[];
  const RECURRENCE_LABELS: Record<string, string> = {
    DAILY: tr("DAILY"),
    WEEKLY: tr("WEEKLY"),
    SELECTED_DAYS: tr("SELECTED_DAYS"),
    ONE_TIME: tr("ONE_TIME"),
  };
  ...
  return (
    <AdminPageShell maxWidth="max-w-3xl" padding="p-6">
      <div dir={dir} className="space-y-6">

        {/* Breadcrumb */}
        <div className="flex items-center gap-2 text-sm">
          <Link href="/admin" className="text-slate-500 hover:text-slate-300">{t("breadcrumbAdmin", { fallback: "لوحة الإدارة" })}</Link>
          <span className="text-slate-700">/</span>
          <Link href="/admin/programs" className="text-slate-500 hover:text-slate-300">{t("breadcrumbPrograms", { fallback: "البرامج" })}</Link>
          <span className="text-slate-700">/</span>
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              {t("presenter")}: <span className="text-slate-200">{program.presenter.name || program.presenter.username}</span>
              {" · "}
              {t("station")}: <span className="font-mono text-cyan-400">{program.station.slug}</span>
            </p>
          </div>
          <div className="flex items-center gap-2 mt-1 flex-shrink-0">
            ...
            {(() => {
              ...
              if (program.validUntil && program.validUntil < now)
                return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium border bg-red-500/10 text-red-400 border-red-500/20">{t("expired")}</span>;
              if (program.validFrom && program.validFrom > now)
                return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium border bg-amber-500/10 text-amber-400 border-amber-500/20">{t("upcoming")}</span>;
              return null;
            })()}
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border ${program.isActive ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-slate-700/50 text-slate-500 border-slate-600/30"}`}>
              {program.isActive ? t("active", { fallback: "نشط" }) : t("inactive", { fallback: "موقوف" })}
            </span>
          </div>
        </div>
        ...
        <section className="bg-slate-800 border border-slate-700/50 rounded-2xl p-5 shadow-xl">
          <h2 className="text-base font-semibold text-slate-200 mb-4">{t("title")}</h2>
          <form action={updateProgram} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-300">{t("descLabel")}</label>
                <input name="description" type="text" defaultValue={program.description ?? ""} placeholder={t("descPlaceholder")} className={inp} />
              </div>
            </div>
            ...
            <div className="border-t border-slate-700/40 pt-4">
              <p className="text-xs font-medium text-slate-400 mb-3 flex items-center gap-2">
                ...
                {t("validityWindow")}
                <span className="text-slate-600 font-normal">{t("emptyIsPermanent")}</span>
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input name="validFrom" type="date" dir="ltr"
                    ...
                    className={inp} />
                  <p className="text-xs text-slate-600">{t("emptyStartsImmediately")}</p>
                </div>
                <div className="space-y-1.5">
                  ...
                  <input name="validUntil" type="date" dir="ltr"
                    ...
                    className={inp} />
                  <p className="text-xs text-slate-600">{t("emptyPermanentProgram")}</p>
                </div>
              </div>
            </div>
        ...
        <section id="schedule-section" className="space-y-4">
          <h2 className="text-base font-semibold text-slate-300">{t("scheduleRules")} ({program.scheduleRules.length})</h2>
          ...
          {program.scheduleRules.map(rule => (
            <div key={rule.id} id={`rule-${rule.id}`} className="bg-slate-800 border border-slate-700/50 rounded-2xl p-5 shadow space-y-4">
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <span className="text-xs text-slate-500 mr-2">
                    · {t("allowConnectMinutesAbbr", { minutes: rule.allowConnectMinutesBefore })} · {rule.timezone}
                  </span>
                </div>
                ...
                <form action={updateScheduleRule} className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3 border-t border-slate-700/40 pt-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select name="recurrenceType" defaultValue={rule.recurrenceType} className={inpSm}>
                      ...
                      <option value="SELECTED_DAYS">{tr("SELECTED_DAYS")}</option>
                      <option value="ONE_TIME">{tr("ONE_TIME")}</option>
                    </select>
                  </div>
                  <div className="space-y-1.5">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs text-slate-400">{t("allowConnectMinutes")}</label>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          <form action={deleteScheduleSlot}>
                            ...
                            <input type="hidden" name="programId" value={programId} />
                            <button type="submit" className="text-xs text-red-400 hover:text-red-300 transition-colors">{t("delete", { fallback: "حذف" })}</button>
                          </form>
                        </div>
                      </div>
                        ...
                        <form action={updateScheduleSlot} className="mt-2 grid grid-cols-2 md:grid-cols-5 gap-2 border-t border-slate-700/30 pt-2">
                          ...
                          {(rule.recurrenceType === "WEEKLY" || rule.recurrenceType === "SELECTED_DAYS") && (
                            <div className="space-y-1 col-span-2 md:col-span-1">
                              <label className="text-xs text-slate-400">{t("day")}</label>
              ...
              <details className="group">
                ...
                <p className="mt-2 text-xs text-slate-500">{t("slotConflictWarning")}</p>
