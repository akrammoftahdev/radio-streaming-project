      ...
      <form action={createProgram} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            <label htmlFor="prog-title" className="text-sm font-medium text-slate-300">
              {t("programTitle")} <span className="text-red-400">*</span>
            </label>
            <input
              id="prog-title"
              name="title"
          ...
          <div className="space-y-1.5">
            <label htmlFor="prog-stationId" className="text-sm font-medium text-slate-300">
              {t("station")} <span className="text-red-400">*</span>
            </label>
            <select
              id="prog-stationId"
              name="stationId"
          ...
          <div className="space-y-1.5">
            <label htmlFor="prog-presenterId" className="text-sm font-medium text-slate-300">
              {t("presenter")} <span className="text-red-400">*</span>
            </label>
            <select
              id="prog-presenterId"
              name="presenterId"
            ...
            >
              {!hasStation ? (
                <option value="">{t("selectStationFirst")}</option>
              ) : !hasEligible ? (
                <option value="">{t("noPresentersLinked")}</option>
              ) : (
                <>
            ...
            {!hasStation && (
              <p className="text-xs text-amber-400/80 mt-1">⬆ {t("selectStationToShowPresenters")}</p>
            )}
            {hasStation && !hasEligible && (
              <p className="text-xs text-red-400/80 mt-1">
                {t("noPresentersLinkedFull")}{" "}
                <a href="/admin/presenters" className="underline hover:text-red-300">{t("presenterEditPage")}</a>.
              </p>
            )}
          </div>
          ...
          {/* Description */}
          <div className="space-y-1.5">
            <label htmlFor="prog-description" className="text-sm font-medium text-slate-300">{t("description")}</label>
            <input
              id="prog-description"
              name="description"
              type="text"
        ...
        <div className="border-t border-slate-700/40 pt-4">
          <p className="text-xs font-medium text-slate-400 mb-3 flex items-center gap-2">
            ...
            {t("validityWindow")} <span className="text-slate-600 font-normal">{t("validityOptional")}</span>
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label htmlFor="prog-validFrom" className="text-sm font-medium text-slate-300">📅 {t("validFrom")}</label>
              <input id="prog-validFrom" name="validFrom" type="date" className={inp} dir="ltr" />
              <p className="text-xs text-slate-600">{t("blankStartsImmediately")}</p>
            </div>
            <div className="space-y-1.5">
              <label htmlFor="prog-validUntil" className="text-sm font-medium text-slate-300">📅 {t("validUntil")}</label>
              <input id="prog-validUntil" name="validUntil" type="date" className={inp} dir="ltr" />
              <p className="text-xs text-slate-600">{t("blankPermanent")}</p>
            </div>
          </div>
        </div>
