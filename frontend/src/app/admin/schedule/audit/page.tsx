...
export async function generateMetadata() {
  const t = await getTranslations("admin.schedule.auditPage");
...
export default async function ScheduleAuditPage({
            ...
            for (const sA of rA.slots) {
              for (const sB of rB.slots) {
                ...
                if (sameDow && sA.startTime && sA.endTime && sB.startTime && sB.endTime &&
                  ...
                  issues.push({
                    severity: "ERROR", type: t("typeA"),
                    description: t("descA", { titleA: a.title, titleB: b.title, station: sName, startA: sA.startTime, endA: sA.endTime, startB: sB.startTime, endB: sB.endTime }),
            ...
            for (const sA of rA.slots) {
              for (const sB of rB.slots) {
                ...
                if (sameDow && sA.startTime && sA.endTime && sB.startTime && sB.endTime &&
                    slotsOverlap(sA.startTime, sA.endTime, sB.startTime, sB.endTime)) {
                  issues.push({
                    severity: "ERROR", type: t("typeB"),
                    description: t("descB", { presenter: pName }),
  ...
  for (const p of programs) {
    ...
    if (!linkedStations || !linkedStations.has(p.stationId)) {
      issues.push({
        severity: "WARNING", type: t("typeC"),
        description: t("descC", { program: p.title, station: p.station?.name ?? p.stationId }),
  ...
  for (const p of programs) {
    if (p.presenter?.presenterMode === "DIRECT_DJ") {
      issues.push({
        severity: "ERROR", type: t("typeD"),
        description: t("descD", { presenter: p.presenter.username, program: p.title }),
  ...
  for (const p of programs) {
    if (p.presenter?.presenterMode === "SINGLE_STATION") {
      ...
      if (stns && stns.size > 1) {
        issues.push({
          severity: "WARNING", type: t("typeE"),
          description: t("descE", { presenter: p.presenter.username, count: stns.size }),
  ...
  for (const p of programs) {
    if (p.scheduleRules.length === 0) {
      issues.push({
        severity: "WARNING", type: t("typeF"),
        description: t("descF", { program: p.title, station: p.station?.name ?? '' }),
        stationName: p.station?.name, programTitle: p.title,
        suggestion: t("suggF"),
      });
    }
  ...
  for (const p of programs) {
    ...
    if (stn && !stn.isActive) {
      issues.push({
        severity: "WARNING", type: t("typeG"),
        description: t("descG", { program: p.title, station: stn.name }),
  ...
  const TYPE_KEY_MAP: Record<string, string> = {
    A: t("typeA"), B: t("typeB"), C: t("typeC"), D: t("typeD"), E: t("typeE"), F: t("typeF"), G: t("typeG")
  };
  const CHECK_TYPES: { key: string; label: string }[] = [
    { key: "A", label: t("legendA") },
    { key: "B", label: t("legendB") },
    { key: "C", label: t("legendC") },
    { key: "D", label: t("legendD") },
    { key: "E", label: t("legendE") },
    { key: "F", label: t("legendF") },
    { key: "G", label: t("legendG") },
  ];
  ...
  const buildUrl = (ov: { severity?: string; station?: string; types?: string; page?: number }) => {
    ...
    const t_p = ov.types     ?? typesParam;    if (t_p)  p.set("types", t_p);
    ...
    <div dir={dir}>
      <AdminPageShell maxWidth="max-w-4xl" padding="p-6 md:p-10">
        ...
        <div className="flex items-center gap-2 text-sm text-neutral-500 mb-6">
          <Link href="/admin" className="hover:text-neutral-300 transition-colors">{t("admin")}</Link>
          <span>/</span>
          <Link href="/admin/schedule" className="hover:text-neutral-300 transition-colors">{t("schedule")}</Link>
          <span>/</span>
          <span className="text-amber-400">{t("audit")}</span>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-neutral-100">{t("titleMain")}</h1>
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 space-y-4">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex bg-neutral-800 border border-neutral-700 rounded-xl p-1 gap-1">
                {[{ v:"all",label:t("all") },{ v:"ERROR",label:t("errors") },{ v:"WARNING",label:t("warnings") }].map(o=>(
              ...
              {allStationNames.length > 0 && (
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-xs text-neutral-500">{t("stationLabel")}</span>
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-xs text-neutral-500 shrink-0">{t("issueType")}</span>
              {CHECK_TYPES.map(ct => {
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: t("criticalErrors"),  count: errors.length,   style: "bg-red-950/40 border-red-800/50 text-red-400" },
              { label: t("warningsCount"),     count: warnings.length, style: "bg-amber-950/30 border-amber-800/40 text-amber-400" },
              { label: t("activePrograms"), count: programs.length, style: "bg-neutral-900 border-neutral-800 text-neutral-400" },
          ...
          {pagedIssues.length === 0 ? (
            <div className="bg-emerald-950/30 border border-emerald-800/40 rounded-2xl px-6 py-8 text-center">
              ...
              <p className="text-emerald-400 font-semibold">{t("noIssues")}</p>
          ...
          <div className="bg-neutral-900/50 border border-neutral-800/50 rounded-2xl px-5 py-4 text-xs text-neutral-500 space-y-1">
            <p className="font-semibold text-neutral-400 mb-2">{t("legendTitle")}</p>
            {CHECK_TYPES.map(ct => (
              <p key={ct.key}>{ct.label}</p>
            ))}
          </div>
