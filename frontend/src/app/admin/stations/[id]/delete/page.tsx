...
export default async function StationDeletePage({
  ...
  if (deps.programs > 0) blockers.push(`${deps.programs} ${t("program")}`);
  ...
  const depRows: DepRow[] = [
    {
      ...
      note:  deps.programs === 0
        ? t("noProgramsNoteDelete", { deleteStation: t("deleteStation") })
        : t("hasProgramsNote"),
      actionSlot: deps.programs > 0 ? (
        <StationCleanupButton
          stationId={stationId}
          action={cleanupStationPrograms}
          buttonLabel={`🗑 ${t("deleteNPrograms", { n: deps.programs })}`}
          variant="danger"
          confirmText={`${t("deleteProgramsConfirm", { n: deps.programs })}`}
        />
      ) : undefined,
    },
    {
      label: t("depPresenters"),
      count: deps.presenterStationLinks,
      safe:  true, // cascades automatically — info only
      note:  deps.presenterStationLinks === 0
        ? t("noPresentersNote")
        : `${t("hasPresentersNote", { n: deps.presenterStationLinks })}`,
      actionSlot: deps.presenterStationLinks > 0 ? (
        <StationCleanupButton
          stationId={stationId}
          action={unlinkStationPresenters}
          buttonLabel={`${t("unlinkNPresenters", { n: deps.presenterStationLinks })}`}
          variant="warning"
          confirmText={`${t("unlinkPresentersConfirm", { n: deps.presenterStationLinks })}`}
        />
      ) : undefined,
    },
    {
      label: t("depSinglePresenters"),
      count: deps.singleStationPresenters,
      safe:  true,
      note:  deps.singleStationPresenters === 0
        ? t("noSinglePresentersNote")
        : `${t("hasSinglePresentersNote", { n: deps.singleStationPresenters })}`,
    },
    {
      label: t("depManagers"),
      ...
      note:  deps.stationManagers === 0
        ? t("noManagersNote")
        : `${t("hasManagersNote", { n: deps.stationManagers })}`,
      actionSlot: deps.stationManagers > 0 ? (
        <StationCleanupButton
          stationId={stationId}
          action={removeStationManagers}
          buttonLabel={`${t("removeNManagers", { n: deps.stationManagers })}`}
          variant="warning"
          confirmText={`${t("removeManagersConfirm", { n: deps.stationManagers })}`}
        />
      ) : undefined,
    },
    {
      label: t("depDjCredentials"),
      count: deps.defaultCredential ? t("exists") : t("notExists"),
      safe:  true, // cascades automatically
      note:  deps.defaultCredential
        ? `${t("hasDjCredentialNote", { username: deps.defaultCredential.djUsername })}`
        : t("noDjCredentialNote"),
      actionSlot: deps.defaultCredential ? (
        <StationCleanupButton
          stationId={stationId}
          action={deleteStationDefaultCredential}
          buttonLabel={t("deleteDjCredentialBtn")}
          variant="warning"
          confirmText={t("deleteDjCredentialConfirm")}
        />
      ) : undefined,
    },
    {
      ...
      note:  deps.recordings === 0
        ? t("noRecordingsNote")
        : `${t("hasRecordingsNote", { n: deps.recordings })}`,
    ...
    },
    {
      ...
      note:  deps.broadcastSchedules === 0
        ? t("noSchedulesNote")
        : `${t("hasSchedulesNote", { n: deps.broadcastSchedules })}`,
    },
  ];
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-6 md:p-10">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm text-neutral-500">
          ...
          <span>/</span>
          <Link href="/admin/stations" className="hover:text-neutral-300 transition-colors">{t("title")}</Link>
          <span>/</span>
          <span className="text-red-400 font-medium">{t("deleteStationTitle")}</span>
        </div>
        ...
        {!deps.isHardDeleteSafe ? (
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl px-6 py-5 space-y-4">
            <div>
              <p className="text-sm font-semibold text-emerald-400 mb-1">✅ {t("readyForDelete")}</p>
              <p className="text-xs text-neutral-400">
                {t("readyForDeleteDesc")}
              </p>
            </div>
            <StationCleanupButton
              stationId={stationId}
